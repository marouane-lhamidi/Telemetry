...
public class MessageServiceImpl implements MessageService {
    ...
    public void initialProcess() {
        //The initial message
        Message initialMessage = new Message(UUID.randomUUID().toString(),"MS1", Ms.MS2, null,0);
    ...
    public void sendingMessage(Message message) {
        if(message.getCounter() < 5)
        {
            ...
            message.setSource(Ms.MS1);

            //Before sending message generating a new Id
            message.setId(UUID.randomUUID().toString());


            ReceivedMessage receivedMessage = new ReceivedMessage(UUID.randomUUID().toString(),"Well received from source : "+message.getSource()+" to destination : "+Ms.MS1 ,message.getSource(),message.getDestination());
